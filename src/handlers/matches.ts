...
composer.callbackQuery("matches:list", async (ctx) => {
  ...
  if (matches.length === 0) {
    await ctx.editMessageText("No upcoming matches right now — check back soon!", {
  ...
  const lines = matches.map(
    (m, i) => `${i + 1}. ${m.home_team} vs ${m.away_team}\n   ${formatDate(m.match_datetime)} · ${m.competition_name}`,
  );
...
composer.command("matches", async (ctx) => {
  ...
  if (matches.length === 0) {
    await ctx.reply("No upcoming matches right now — check back soon!", {
      reply_markup: inlineKeyboard([[inlineButton("⬅️ Back to menu", "menu:main")]]),
    });
    return;
  }
